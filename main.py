...
modeType = 0 #Sets what the state of the system is for example: Active, Marked, Already marked
counter = 0
id = -1
...
while True:
    ...
    #if a face is detected execute all the logic. Otherwise reset the system
    if faceCurFrame:
        ...
        if counter != 0:
            #in the first frame only, download the student's data from the database and update it
            if counter == 1:
                ...
                #if 60 seconds have passed since the student last took attendance, update it. Otherwise don't update anything
